...
parser.add_argument("-oldAmp", "--Old_amplifier", help = "*REQUIRED INPUT*   Indicate the old amplifier initiator sequence you want to replace in these probes. Supports B1-B5,B7,B9,B10,B11,B13,B14,B15,B17,S10,S23,S25,S34,S35, and S41. Example: -oldAmp B1 ")
parser.add_argument("-newAmp", "--New_amplifier", help = "*REQUIRED INPUT*   Indicate the replacement amplifier. Supports B1-B5,B7,B9,B10,B11,B13,B14,B15,B17,S10,S23,S25,S34,S35, and S41. Example: -newAmp B5 ")
...
roots = cod()
MEDIA_ROOT,BLAST_ROOT,MEDIA_FASTA,MEDIA_OPOOL,MEDIA_OLIGO = roots[0],roots[1],roots[2],roots[3],roots[4]
...
def swap(seq,old,new):
    ...
    while p < seqs:
        for sequence in seqseq:
            if str(amps[old][0]) in str(sequence):
                new0 = str(sequence).replace(str(amps[old][0]),str(amps[new][0]))
                #print(new0,seqs)
                name = newname(seqnm[p],old,new)
                ...
                new1 = str(sequence).replace(str(amps[old][1]),str(amps[new][1]))
                #print(new1)
                name = newname(seqnm[p],old,new)
    ...
    x.to_excel(os.path.join(MEDIA_OPOOL,(name+"_initiatorswap.xlsx")))

    y = pd.DataFrame(data=newoligoseqs, columns=["Name","Sequence","Scale","Purification"])
    y = y.set_index("Name")
    y.to_excel(os.path.join(MEDIA_OLIGO,(name+"_initiatorswap.xlsx")))
...
if args.New_amplifier:
    new = str(args.New_amplifier).upper()
if args.in_file:
